...
export function isValidSignupCode(value) {
...
export function friendlyAuthError(error, fallback) {
  ...
  if (/signup code|six-digit/i.test(message)) return message;
...
export function getSignUpErrorMessage(data, error) {
  if (error) {
    return friendlyAuthError(
      error,
      'We could not create that account. Check the email, passwords, and signup code, then try again.',
    );
  }
